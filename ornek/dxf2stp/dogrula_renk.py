...
P = dict(D.VARSAYILAN)
ciz = D.Cizim('fikstur_2a.dxf', P)
ciz.atom = [q for q in ciz.atom if -442150 <= (q["kutu"][0]+q["kutu"][2])/2 <= -441350
            and 420000 <= (q["kutu"][1]+q["kutu"][3])/2 <= 424300]
ciz.segment = [s for q in ciz.atom for s in q["seg"]]
bol, cer, _ = D.bolgele_renk(ciz, P)
nes = D.nesne_esle_renk(bol, P)
for n in nes:
    try: D.nesne_kur(n, P)
...
fig.tight_layout(); fig.savefig("dogrulama_renk.png", dpi=85); plt.close(fig)
print("dogrulama_renk.png yazildi")
