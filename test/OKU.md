...
| `cizim_cakisma_denetimi.py` | Üretilmiş resimlerde **yazı yazının üstüne binmiş mi**, **yazı konturun üstüne binmiş mi**. Yazının kapladığı yer hesapla değil ÖLÇÜLEREK bulunur; ölçü blokları da açılıp içlerindeki yazı sayılır. Doğrusu ikisinde de sıfır. | `python test/cizim_cakisma_denetimi.py cikti` |
| `yontem_denetimi.py` | Parçanın abkantta bükülüp bükülemeyeceği doğru saptanıyor mu: kanat ve yarıçap sınırlarının iki yanı, gerçek parçalardan ölçülmüş değerler, sebebin yazılması ve sınırların ayarlanabilmesi. | `python test/yontem_denetimi.py` |
