...
| **6 AÇINIM** | **bükümlü sac parçaları program kendisi bulur** (silindirik yüzeylerden; parça başına ~20 ms, açınım hesabının binde biri) ve **listede yalnız onlar kalır**, hepsi seçili gelir — düz sac ve sac olmayan parçalar listeye alınmaz. Açınım, detay resmiyle **aynı adı** taşır, sonuna `_acinim` eklenir. Bükümlü sacların kesim konturu + **büküm yöntemi** (abkant / rollform / silindir): kanat V kalıbın ağzını tutamayacak kadar kısaysa ya da iç yarıçap fazla küçükse "abkantta yapılamaz" der, ölçüsüyle: dış kontur, kenar kesikleri, delikler gerçek yerlerinde + büküm çizgileri ve büküm tablosu. K-faktörü ayarlanır ve saklanır. Hesap güvenilir değilse açınım **verilmez**, sebebi yazılır |
| **7 PAFTA** | 1:1 resimlerin **kendi dosyasına** standart A3 pafta sekmesi ekler — **kâğıdın yönünü (yatay/dikey) program seçer**, parça hangi yönde daha büyük görünüyorsa o; detay resimleri ve açınımlar birlikte listelenir, ikisinin de paftası ve PDF'i çıkar. **Firma anteti** kullanılabilir (`EXE_YAP.bat` → 2): çerçeve, bölge işaretleri, antet ve firma logosu firmanın kendi DXF'inden gelir, Scale / Weight / Material / Part Name / Drawing No / FILE kutularını Pi3D doldurur, tarih-çizen-onaylayan programda sorulur ve saklanır (kopya yok — kopya ile asıl resim zamanla ayrışıyordu): kenardan 15 mm pay, sağ alt köşede 150×100 mm boş antet alanı (oraya asla resim gelmez), kenarlarda bölge işaretleri. Model sekmesi 1:1 kalır. Ölçek standart merdivenden seçilir; **ölçü rakamları her zaman 1:1 kalır.** PDF kendiliğinden basılmaz; istendiğinde `PDF` klasörüne `..._A3.pdf` adıyla yazılır |
| **8 LAZER** | **kesim resimleri** (`..._Lzr.dxf`): yalnız kesim konturu — dış kontur ve delikler, 1:1, tek katmanda, hepsi kapalı çokgen. Büküm çizgisi, tablo, ölçü, yazı, çerçeve ve antet **yok**; **paftaya alınmaz, PDF'i basılmaz** (CAM her çizgiyi kesim yolu sayabilir). Açınımı çıkanlar seçili gelir, diğer sac parçaları kullanıcı seçer. Düz sacın konturu kendi yüzünden çıkarılır ve hacimle çapraz denetlenir; tutmazsa **verilmez**. `LAZER.csv` ile birlikte |
